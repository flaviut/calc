...
function eSeriesToValues(series: number[], min: number, max: number): number[] {
    ...
    for (let i = startLog; i < endLog; i++) {
        result = [...result, ...series.map((v) => v * Math.pow(10, i))]
    }
...
function eSeriesToText(
    ...
    r_min: string | number | mathjs.Unit,
    r_max: string | number | mathjs.Unit,
): string {
    return `[${eSeriesToValues(
        eSeries[series],
        // typing is not quite correct: mathjs.unit() will accept unit parameters
        mathjs.number(mathjs.unit(r_min as string), "ohm") as number,
        mathjs.number(mathjs.unit(r_max as string), "ohm") as number)
        .map((val) => presentUnit(val, "ohm")).join(", ")}]`
}
...
function calculateBestResistors(resistors: mathjs.Matrix, vIn: mathjs.Unit, vOut: mathjs.Unit) {
    let best = { v: Number.NEGATIVE_INFINITY, r1: -1, r2: -1 }

    const vInNum = mathjs.number(vIn, "V")
    const vOutNum = mathjs.number(vOut, "V")
    const resistorsNum = resistors.map((val) => mathjs.number(val, "ohm")).toArray() as number[]

    for (const r1 of resistorsNum) {
        for (const r2 of resistorsNum) {
...
const VoltageDividerPage: React.FunctionComponent = () => {
    const [scope, setScope] = useState({
        v_in: parseInput("12 V", "V"),
        v_out: parseInput("3.3 V", "V"),
        r_min: parseInput("10 kohm", "ohm"),
        r_max: parseInput("1 Mohm", "ohm"),
        resistor_values: parseInput(eSeriesToText(24, "10 kohm", "1 Mohm"), "ohm[]"),
        r1: parseInput("240 kohm", "ohm"),
        r2: parseInput("91 kohm", "ohm"),
    })

    const [selectedSeries, setSelectedSeries] = useState<keyof typeof eSeries | 'custom'>(24)

    const updateResistors = React.useCallback((newScope: GenericScope) => {
        if (newScope.resistor_values.isError || newScope.v_in.isError || newScope.v_out.isError) {
    ...
    const updateSeries = React.useCallback((series: keyof typeof eSeries, newScope: typeof scope) => {
    ...
    const updateVout = React.useCallback((newScope: GenericScope) => {
        ...
        setScope({
            ...newScope,
            v_out: parseInput(presentUnit(evaluateVoltageDivider(
                mathjs.number(newScope.r1.value as mathjs.Unit, "ohm"),
                mathjs.number(newScope.r2.value as mathjs.Unit, "ohm"),
                mathjs.number(newScope.v_in.value as mathjs.Unit, "V")), "V"), "V")
        } as typeof scope)
    }, [setScope])
